...
RUN apt-get update -qq && apt install -y -qq locales curl ca-certificates net-tools zip make unzip zsh subversion git software-properties-common jq zlib1g-dev libsqlite3-dev python3-pip yamllint pylint tidy clang-tidy apache2-dev libapr1-dev libaprutil1-dev libapache2-mod-perl2 libapache2-mod-apreq2 libapache2-request-perl
WORKDIR /tmp
RUN curl -L https://github.com/koalaman/shellcheck/releases/download/v0.8.0/shellcheck-v0.8.0.linux.x86_64.tar.xz | tar -xJf - && mv shellcheck-v0.8.0/shellcheck /usr/local/bin
...
RUN bash -c '. ~/.asdf/asdf.sh; mkdir -p ~ubuntu/bin; echo -e "#!/bin/bash\nexec /usr/bin/curl -k \$@" > ~/bin/curl; chmod +x ~/bin/curl; for pkg in dotnet-core golangci-lint; do asdf plugin-add $pkg; v="$(asdf list-all $pkg | tail -n 1)"; asdf install $pkg $v; echo $pkg $v >>~/.tool-versions; done; asdf plugin-add nodejs && asdf install nodejs 18.9.0 && echo nodejs 18.9.0 >>~/.tool-versions'
...
USER root
ENV NODE_PATH=./node_modules:$HOME/.asdf/installs/nodejs/18.9.0/lib/node_modules
ENV TERM=xterm-256color
ENV LANG=en_US.UTF-8
RUN localedef -i en_US -c -f UTF-8 -A /usr/share/locale/locale.alias en_US.UTF-8
RUN curl -L https://github.com/hadolint/hadolint/releases/download/v2.10.0/hadolint-Linux-x86_64 -o /usr/local/bin/hadolint && chmod 0755 /usr/local/bin/hadolint
RUN mkdir /src && chown ubuntu:ubuntu /src
USER ubuntu
ENV USER=ubuntu
RUN ln -s /home/bin/ubuntu/node_modules /tmp/node_modules && git config --global --add safe.directory /src
WORKDIR /src
