...
RUN bash -c '. ~/.asdf/asdf.sh; npm config set strict-ssl false && npm install -g eslint typescript navigator jsdom jquery && npm install -g --save-dev @typescript-eslint/parser @typescript-eslint/eslint-plugin remark-cli remark-preset-lint-consistent remark-preset-lint-recommended remark-preset-lint-markdown-style-guide remark-validate-links remark-lint-list-item-indent'
RUN pip3 install flake8 black
RUN cpan -f install sealed Algorithm::Diff LCS::BV LCS::XS B::Lint IO::Select URI Term::ReadKey Perl::Critic
