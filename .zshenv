...
PTYON=(sudo git gpg op ssh ansible-vault ortpasswd otp-sha1)
...
# PATH
for p in /sbin /usr/sbin /usr/local/bin "$(go env GOPATH 2>/dev/null)/bin" ~/{.local,.krew}/bin ~/.dotnet/tools ~/bin
do
