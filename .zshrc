...
alias report_node_inventory='join -j1 <(join -j 1 <(perl -nale "/ provisioned-cpu / ... /Running/ and !/Running/ and length and print \"\$ARGV:\$F[0] \$F[-1]\"" /tmp/k8s/reports/nodes/*/* | sort) <(perl -nale "/ provisioned-mem / ... /Running/ and !/Running/ and length and print \"\$ARGV:\$F[0] \$F[-2]\$F[-1]\"" /tmp/k8s/reports/nodes/*/* | sort)) <(perl -nale "/ age / ... /Running/ and !/Running/ and length and print \"\$ARGV:\$F[0] \$F[-1]\"" /tmp/k8s/reports/nodes/*/* | sort) | sed -e "s!/tmp/k8s/reports/nodes/!!" | sort -k3nr | (echo "AWSREGION/ORGNAME.EKSCLUSTER:HOSTNAME CPU RAM AGE"; cat)'
