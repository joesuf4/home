...
alias report_node_inventory_static='join -j 1 -a 1 <(join -j 1 <(join -j 1 <(_report_filter_block nodes provisioned-cpu "" :) <(_report_filter_block nodes provisioned-mem "" :)) <(_report_filter_block nodes age "" :)) <(_report_filter_block nodes -price "" :) | sort -k3nr | perl -pale "@F == 5 and \$_ .= sprintf \" %.2f %.2f\", \$F[3]*\$F[4]*24, 30*\$F[4]*24" | (echo -e "AWSREGION\tAWSORGID\tBXORGNAME\tEKSCLUSTER\tEC2HOSTNAME\tCPU\tRAM\tAGE\tPRICE\tTCO\tMONTHLY"; perl -nale "BEGIN{\$,=\"\\t\"} splice @F, 0, 1, split m![/:]!, \$F[0]; splice @F, 1, 1, split /[.]/, \$F[1]; splice @F, 1, 0, grep chomp, qx([ -z \"$NOSHELL\" ] && $SHELL -ic \"bcs get-account-number \$F[1]\" || echo PLACEHOLDER); print @F")'
