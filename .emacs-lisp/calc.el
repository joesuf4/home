...
(defun calc-init-base ()
  ...
  ;; Define some aliases for Emacs 20 which will work with this
  ;; version of Calc, which has been modified for Emacs 21.
  (when (< emacs-major-version 21)
    (defalias 'field-string 'buffer-string)
    (defun field-beginning (&optional pt)
      (if pt
	  pt
	(point-min)))
    (defun field-end (&optional pt)
      (if pt
	  pt
	(point-max))))
